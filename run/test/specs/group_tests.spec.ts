...
async function mentionsForGroups(platform: SupportedPlatformsType) {
  ...
  await runOnlyOnAndroid(platform, () =>
    device1.selectByText("Contact mentions", userB.userName)
  );
  ...
  await runOnlyOnAndroid(platform, () =>
    device1.selectByText("Contact mentions", userC.userName)
  );
  await device1.clickOnElement("Send message button");
  await device1.waitForTextElementToBePresent({
    strategy: "accessibility id",
  ...
  await runOnlyOnAndroid(platform, () =>
    device3.selectByText("Contact mentions", userA.userName)
  );
