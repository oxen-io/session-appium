...
async function disappearingVideoMessage1o1Ios(platform: SupportedPlatformsType) {
...
async function disappearingVideoMessage1o1Android(platform: SupportedPlatformsType) {
  const { device1, device2 } = await openAppTwoDevices(platform);
  // Create user A and user B
  const [userA, userB] = await Promise.all([
    newUser(device1, 'Alice', platform),
